...
from models import *
# from utils_test import evaluate_normal, evaluate_pgd, evaluate_autoattack, evaluate_cw, evaluate_eot_pgd, evaluate_eot_pgd_l2
from attacks import *
...
if __name__ == '__main__':
    ...
    if config.Operation.Validate_Best == True:
        ...
        if device == 'cuda':
            ...
            checkpoint_best = torch.load(os.path.join(check_path, 'model_best.pth.tar'),map_location=torch.device('cpu'))
        ...
        if config.Operation.Validate_EOT_PGD:
            for pgd_param in config.ADV.pgd_test:
                pgd_acc = evaluate_eot_pgd(net, test_loader, pgd_param[1], pgd_param[2], pgd_param[0], config.ADV.eot)
                logger.info(f"EOT_PGD_attack:[nb_iter:{pgd_param[0]},eps:{pgd_param[1]},step_size:{pgd_param[2]},eot:{config.ADV.eot}]->eot_pgd_acc: {pgd_acc: .2f}")
        if config.Operation.Validate_EOT_PGD_L2:
            for pgd_param in config.ADV.pgd_test:
                pgd_acc = evaluate_eot_pgd_l2(net, test_loader, pgd_param[1], pgd_param[2], pgd_param[0], config.ADV.eot)
                logger.info(f"EOT_PGD_attack (L2):[nb_iter:{pgd_param[0]},eps:{pgd_param[1]},step_size:{pgd_param[2]},eot:{config.ADV.eot}]->eot_pgd_acc: {pgd_acc: .2f}")
    ...
    if config.Operation.Validate_Last == True:
        ...
        if device == 'cuda':
            ...
            checkpoint_last = torch.load(os.path.join(check_path, 'checkpoint.pth.tar'),map_location=torch.device('cpu'))
