...
def evaluate_pgd_torch_attacks(net: nn.Module, test_loader: DataLoader, eps, step, iter) -> float:
    net.eval()
    adv_correct = 0
    total = 0
    progress_bar = tqdm(total=len(test_loader), desc='Testing-PGD>>')
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        inputs, targets = inputs.to(device), targets.to(device)
        total += targets.size(0)
        attack = torchattacks.PGD(net, eps=eps/255, alpha=step/255, steps=iter, random_start=True)
...
def evaluate_EOT_pgd_torch_attacks(net: nn.Module, test_loader: DataLoader, eps, step, iter, eot) -> float:
    net.eval()
    adv_correct = 0
    total = 0
    progress_bar = tqdm(total=len(test_loader), desc='Testing-PGD>>')
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        inputs, targets = inputs.to(device), targets.to(device)
        total += targets.size(0)
        attack = torchattacks.EOTPGD(net, eps=eps/255, alpha=step/255, steps=iter, eot_iter=eot, random_start=True)
        adv_images = attack(inputs, targets)
        with torch.no_grad():
            adv_outputs = net(adv_images)
        _, predicted = adv_outputs.max(1)
